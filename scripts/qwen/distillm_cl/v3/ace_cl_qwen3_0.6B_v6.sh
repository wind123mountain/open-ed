...
data=$1
perm=$2
pair_mode=${3:?Usage: $0 <dataset> <perm> <pair_mode> (same_prefix|cross_prefix|mixed|random)}
...
MAX_LENGTH=640
# runtime
SAVE_PATH="${BASE_PATH}/results/qwen3/v6/${data}_${perm}_0.6B_cl_${pair_mode}"
# seed
SEED=42
...
NUM_TASKS=5
START_TASK=0  # Change to resume from a later task (e.g. 3 if tasks 0-2 are done)

# Initial peft path: empty = task 0 starts with a fresh LoRA on the base model.
# When resuming (START_TASK > 0), auto-detect the previous task's checkpoint.
CURRENT_PEFT_PATH=""
...
# Clean up previous run (skip if resuming from a later task)
...
for TASK_ID in $(seq ${START_TASK} $((NUM_TASKS - 1))); do
    ...
    DATA_DIR="${BASE_PATH}/processed_data/${data}_v6_${pair_mode}_${perm}/${TASK_ID}/qwen/"
