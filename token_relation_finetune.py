...
def relation_loss_fn(student_hidden, teacher_hidden, attention_mask=None):
    ...
    if attention_mask is not None:
        mask = attention_mask.unsqueeze(-1) * attention_mask.unsqueeze(1)
        mask = mask.float()
        
        student_relation = student_relation * mask
        teacher_relation = teacher_relation * mask

    if attention_mask is not None:
        loss = F.mse_loss(student_relation, teacher_relation, reduction='sum')
        loss = loss / mask.sum().clamp(min=1.0) # Tránh chia cho 0
    else:
        loss = F.mse_loss(student_relation, teacher_relation)

    return loss

def compute_overall_relation_loss_loss(attention_mask, s_hidden_states, t_hidden_states, args):
    loss = 0
    for s_layer, t_layer in zip(args.student_layer_mapping, args.teacher_layer_mapping):
        loss += relation_loss_fn(s_hidden_states[s_layer], t_hidden_states[t_layer], attention_mask)
